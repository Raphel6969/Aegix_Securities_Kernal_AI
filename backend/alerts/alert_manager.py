...
class AlertManager:
    ...
    def _init_db(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Migrate old db schema if columns are missing
            try:
                conn.execute("ALTER TABLE webhooks ADD COLUMN trigger_safe BOOLEAN DEFAULT 0")
            except Exception:
                pass
            try:
                conn.execute("ALTER TABLE webhooks ADD COLUMN trigger_suspicious BOOLEAN DEFAULT 0")
            except Exception:
                pass
            try:
                conn.execute("ALTER TABLE webhooks ADD COLUMN trigger_malicious BOOLEAN DEFAULT 1")
            except Exception:
                pass
            conn.commit()
